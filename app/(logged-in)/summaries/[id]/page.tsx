    ...
    if (isStructured) {
      const sections = text
        ...
        .map((section) => {
          ...
          return [titleLine.trim(), ...rest.filter((line) => line.trim())].join(
            " "
          );
        });

      return sections.join(" ").trim().split(/\s+/).filter(Boolean).length;
    }

    return text.trim().split(/\s+/).filter(Boolean).length;
  };

  const computedWordCount = calculateWordCount(summary_text);
  const readingTime = Math.ceil(computedWordCount / 200); // 200 wpm average
